...
- `POST /post/` — cria um post _(somente professores)_. Retorna `201`. Requer `Authorization: Bearer <token>`.
- `PUT /post/:id` — atualiza um post existente _(somente professores)_. Retorna `200`. Requer `Authorization: Bearer <token>`.
- `DELETE /post/:id` — remove um post _(somente professores)_. Retorna `200`. Requer `Authorization: Bearer <token>`.
...
- O repositório de usuários usa `findByUsernameAndRole` para impedir cadastro duplicado com mesmo `username` e mesmo papel (professor/aluno).
